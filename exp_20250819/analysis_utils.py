...
def _read_sample_data(sample, assembler, classification, binning, plaseval_results_dir, alpha):
        ...
        if max_idx is None or idx < max_idx:
            ...
            for binning,classification in product(BINNING, CLASSIFICATION):
                sample_data = _read_sample_data(
                    sample, assembler, classification, binning, in_results_dir, alpha
                )
...
if __name__ == "__main__":
    ...
    parser_csv.add_argument("-a", "--alpha", type=str, help="Alpha parameter for PlasEval")
