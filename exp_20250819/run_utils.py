...
def _create_convert_bins_to_plaseval_command(sample_data_dict, args, ground_truth=False):
    """ Create the command to convert a bins file from PlasMerge format to PlasEval format
    Input:
    - sample_data_dict: dicionary generated by _read_samples_data
    - args: (argparse object) argument for a command
    - ground_truth: (bool) if True, binning is set to ground_truth and merged_statu to unmerged
    Output:
...
def convert_bins_to_plaseval(sample_data_dict, args, ground_truth=False):
    ...
    convert_cmd,in_files,out_file = _create_convert_bins_to_plaseval_command(
        sample_data_dict, args, ground_truth=ground_truth
    )
    if os.path.exists(out_file):
...
def _run_plaseval_eval(args):
    ...
    if args.rerun and _read_alpha(sample_data_dict, args) not in [INPUT_FILE, EVAL_CMD]:
        return
    # Converting merged bins and unmerged ground truth bins into PlasEval format
    convert_bins_to_plaseval(sample_data_dict, args, ground_truth=False)
    convert_bins_to_plaseval(sample_data_dict, args, ground_truth=True)
    # Running PlasEval
